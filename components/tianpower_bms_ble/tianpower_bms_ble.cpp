...
void TianpowerBmsBle::decode_mosfet_status_data_(const std::vector<uint8_t> &data) {
  ...
  uint32_t combined_alarms = (uint32_t(high_alarm) << 16) | low_alarm;
  this->publish_state_(this->alarm_bitmask_sensor_, (float) combined_alarms);
...
void TianpowerBmsBle::decode_cell_voltages_data_(const uint8_t &chunk, const std::vector<uint8_t> &data) {
  ...
  if (chunk == 0 || (current_time - this->last_cell_voltages_chunk_timestamp_) > 5000) {
    ...
    this->sum_cell_voltage_ = 0.0f;
    this->count_cell_voltage_ = 0;
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    if (cell_voltage > 0) {
      this->sum_cell_voltage_ += cell_voltage;
      this->count_cell_voltage_++;
    }
  ...
  if (this->cell_voltages_chunks_received_ == 0b11) {
    ...
    if (this->count_cell_voltage_ > 0) {
      this->publish_state_(this->average_cell_voltage_sensor_, this->sum_cell_voltage_ / this->count_cell_voltage_);
    }
